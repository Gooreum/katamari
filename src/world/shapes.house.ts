...
export const HOUSE_BUILDERS: Record<ShapeIdHouse, () => BufferGeometry> = {
  ...
  고무오리: () => {
    ...
    // 몸통 — 0.46 높이 · 모따기 0.45 는 화면에서 「부피 없는 납작한 판」이었다(트랙 D 2회차).
    // 사진의 몸통 높이(길이의 0.51)에 맞춰 키우고 모따기를 줄여 부피를 준다
    const body = warp(soft(0.86, 0.56, 0.60, 0.30), (x, y, z) => {
    ...
    return assemble([
      part(body, WHITE, [-0.07, 0.28, 0]),
      // ① 머리 — 몸통 앞쪽 위에 묻힌 큰 공
      part(new SphereGeometry(HR, 16, 10), WHITE, [0.128, 0.854 - HR, 0]),
      // ② 넓적한 부리
      // 부리 — 넓적한 주걱. 모따기가 크면 「작은 육각 뭉치」가 된다(트랙 D)
      part(soft(0.12, 0.055, 0.26, 0.18), [0.91, 0.42, 0.95], [0.44, 0.545, 0]),
      // ④ 눈 — 까만 세로 타원
      ...([1, -1] as const).map((k) =>
        part(new SphereGeometry(0.04, 8, 6).scale(0.5, 1.1, 0.8), INK, [0.36, 0.60, k * 0.13], [0, k * -0.6, 0])),
      // 눈 흰자 테 — 검은 점만 있으면 「점」이다(트랙 D)
      ...([1, -1] as const).map((k) =>
        part(new SphereGeometry(0.052, 8, 6).scale(0.4, 1.05, 0.85), [1.25, 1.25, 1.2], [0.352, 0.60, k * 0.128], [0, k * -0.6, 0])),
  ...
  수건: () => {
    ...
    /**
     * **판이 아니라 천으로 읽혀야 한다.** 1회차엔 「수건」이었는데 2회차에 「책 · 공책」으로 뒤집혔다
     * (2026-09-16). 곧은 모서리에 곧은 줄무늬가 겹치니 책장이 됐다. 고친 것:
     *   - 모서리를 크게 둥글린 판(`soft`)으로 — 천은 모서리가 서지 않는다
     *   - 접힌 쪽 등을 굵게, 반대쪽은 한 겹이 **흘러내려 아래로 처지게**
     *   - 위 겹을 아래 겹보다 조금 작고 비스듬히 얹어 층이 어긋나게
     */
    return assemble([
      part(soft(0.99, T * 0.5, D - T, 0.45), [0.94, 0.94, 0.92], [0.01, T * 0.25, T / 2 + 0.02], [0, 0.04, 0]),
      part(soft(0.96, T * 0.55, D - T * 2, 0.45), WHITE, [0, T * 0.62, T / 2], undefined, TILE.TOWEL),
      // ② 접힌 쪽 — 굵게 말린 등
      part(new CylinderGeometry(T * 0.5, T * 0.5, 0.95, 8), [0.95, 0.95, 0.93], [0, T * 0.6, -D / 2 + T * 0.9], [0, 0, Math.PI / 2]),
      // 흘러내린 한 겹 — 반대쪽 끝에서 바닥으로 처진다
      part(soft(0.93, T * 0.35, 0.10, 0.45), [0.92, 0.92, 0.90], [0.01, T * 0.18, D / 2 + 0.02], [0.25, 0, 0]),
    ]);
  },
