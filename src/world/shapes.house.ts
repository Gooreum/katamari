...
export const HOUSE_BUILDERS: Record<ShapeIdHouse, () => BufferGeometry> = {
  ...
  /**
   * 계란 — **사진에서 잰 값으로 다시 만들었다.** 2회차 판정이 「대칭 타원·조약돌」이라 다시 쟀다.
   * 근거: `.design-bounce/ref/계란/` (뭉툭한 끝으로 세운 흰 달걀 정측면)
   *
   * 앞의 것은 반타원 둘을 «가운데»(0.46)에서 이었다. 그래서 뭉툭한 쪽 1/4 폭이 0.89,
   * 뾰족한 쪽이 0.84 — 5% 차이라 어느 쪽이 뭉툭한지 화면에서 안 보였다. 사진을 픽셀로 다시 재니:
   *   ① 폭 : 길이 = **0.74**
   *   ② 가장 넓은 곳이 **뭉툭한 끝에서 0.38** — 가운데가 아니라 아래쪽에 있다
   *   ③ 뭉툭한 쪽 1/4 지점 폭 **0.955**, 뾰족한 쪽 1/4 지점 **0.75** — 20% 차이가 나야 달걀로 읽힌다
   * 반타원(지수 2)으로는 이 비가 안 나온다. 두 반쪽에 지수를 따로 준다 —
   * 뭉툭한 쪽 2.6(더 배가 부르다) · 뾰족한 쪽 1.6(더 빨리 좁아진다).
   * 바닥에서는 **옆으로 눕는다**. 치수는 길이 = 1 로 쓴다.
   */
  계란: () => {
    const R = 0.37, WIDE = 0.38;
    const pts = Array.from({ length: 17 }, (_, k) => {
      // 0 = 뭉툭한 끝, 1 = 뾰족한 끝. 양 끝을 촘촘히 뽑는다 — 고르게 뽑으면 끝 두 점 사이가 원뿔로 뾰족해졌다
      const t = (1 - Math.cos(Math.PI * k / 16)) / 2;
      const u = t < WIDE ? (WIDE - t) / WIDE : (t - WIDE) / (1 - WIDE);
      const e = t < WIDE ? 2.6 : 1.6;                    // ③ 반쪽마다 다른 지수
      const w = Math.sqrt(Math.max(0, 1 - Math.pow(u, e)));
      return new Vector2(Math.max(0.001, R * w), t);
    });
    // 돌림축 y 를 −x 로 눕힌다(뭉툭한 끝 +x). 껍질 얼룩은 옅게 — 세면 메추리알이 된다
    return assemble([
      part(new LatheGeometry(pts, 16), [1.0, 0.99, 0.95], [0.5, R, 0], [0, 0, Math.PI / 2], TILE.EGG),
    ]);
  },
