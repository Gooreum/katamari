...
export const SHAPE_COLOR: Record<string, readonly number[]> = {
  ...
  // ── 버킷 1 (2~4cm) ──────────────────────────────────────────
  // 지우개는 사진(MONO)대로 크림 고무 — 색은 슬리브 인쇄가 낸다
  주사위: [0], 나사: [6], 압핀: [8, 10, 13], 지우개: [0], 각설탕: [0],
  사탕: [8, 10, 16], 성냥: [2],
  // ── 버킷 2 (4~8cm) ──────────────────────────────────────────
  크레용: [8, 9, 10, 11, 14, 16], 캐러멜: [17], 체온계: [0], '간장 팩': [0],
  청개구리: [12], 성냥갑: [8], 건전지: [0], 화투: [0],
  // ── 버킷 3 (8~16cm) ─────────────────────────────────────────
  소시지: [0], 껌: [13, 0], 달팽이: [1], '캐러멜 상자': [0],
  // 사과·찻잔은 인쇄(세로 줄 · 남색 붓무늬)가 색을 정한다 — 흰 팔레트
  사과: [0], 찻잔: [0], 전구: [0],
  // ── 버킷 4 (16~32cm) ────────────────────────────────────────
  찌라시: [0], 신문: [0], 연필깎이: [0],
  /**
   * **검정(5) → 밝은 회색(3) → 흰색(0).** 인쇄는 팔레트 색에 곱해지는 것이라
   * 본체가 검으면 아무리 그려도 안 보인다 — 버튼판을 넣어도 통째로 검은 덩어리다.
   * 앞면 배치는 인쇄(`TILE.RC`)가, 몸통 검정은 정점색이 칠한다(사진 기준 재작업, 2026-09-10).
   */
  'RC 컨트롤러': [0], 접시: [0],
  // 슬리퍼는 꽃무늬 인쇄가 색을 정한다 — 흰 팔레트
  슬리퍼: [0], '두루마리 휴지': [0], 우유팩: [0],
  // ── 버킷 5 (30~60cm) ────────────────────────────────────────
  // 방석은 사진(탁한 팥색 바탕 잔꽃)대로 분홍을 첫 색으로 — 손배치·시트는 첫 색을 쓴다
  방석: [15, 14, 19], 백팩: [8, 5],
  // 휴지통·전화기는 인쇄(꽃무늬·다이얼)가 색을 정한다 — 팔레트는 흰색에 둔다(건전지와 같다).
  // 전화기의 검정은 정점색이 칠한다. 주전자는 사진대로 샴페인 금색 알루마이트(미색 1)
  휴지통: [0], 전화기: [0], 밥솥: [0],
  화분: [0], 주전자: [1],
  // ── 버킷 6 (60cm~1.2m) ──────────────────────────────────────
  // 스툴에 초록(11)·빨강(8)을 주면 검은 쇠다리와 흰 발끝 마개까지 같이 물든다(2회차 트랙 D).
  // 사진은 초록 비닐 좌판에 검은 다리다 — 팔레트는 흰색, 좌판 색은 계수로.
  고양이: [18, 5, 0], 의자: [0], 스툴: [0], 텔레비전: [7], 서랍장: [7],
  // 스탠드는 상아색 갓 · 꿀색 나무 기둥 · 황동 받침이 한 물건에 있다 — 흰 팔레트에 계수로 칠한다
  스탠드: [0], 물뿌리개: [0],
  // ── 방 정체성 (부엌·화장실·아이 방) ─────────────────────────
  // 계란은 흰색 하나뿐이다. 색을 여럿 주면 파란 달걀이 나온다 —
  // 「실제로 한 색뿐인 것은 하나만 둔다」는 이 표의 규칙 그대로다.
  계란: [0], 밥공기: [0], 젓가락: [0], 숟가락: [6], 당근: [0], 냄비: [6], 도마: [0],
  비누: [0, 15, 13], 고무오리: [10], 칫솔: [0], 수건: [0, 13, 15],
  // 구슬 · 딱지 · 공책은 사진 기준 재작업에서 인쇄가 색을 정한다 — 흰 팔레트
  구슬: [0], '장난감 블록': [8, 10, 11, 14], 딱지: [0],
  공책: [0], 곰인형: [2, 18],
  // ── 가구 (손배치 전용 — 난수로는 안 뽑힌다) ────────────────
  // 나무 계열은 7(나무), 종이 더미는 0(종이 흰색). 손배치는 첫 색만 쓴다
  TV장: [7], 책장: [7], 밥상: [7], 신문더미: [0], 화분대: [7], 방석더미: [0],
  // ── 거실 소품 ────────────────────────────────────────────────
  // 책은 표지 색이 여럿이라야 책장 한 칸이 책 «더미»로 보인다 — 한 색이면 벽돌담이다.
  // 인쇄(TILE.BOOK 등)가 팔레트 색에 곱해지므로 어두운 색은 인쇄를 먹는다. 밝은 쪽만 준다
  // 사진의 책은 탁한 적갈색·크림 — 첫 색(시트·손배치)을 적갈색으로. 비디오테이프는 몸통 검정을 정점색이
  // 칠하고 창·라벨은 인쇄가 그리므로 흰 팔레트(검정 팔레트면 인쇄까지 전부 검게 곱해진다)
  책: [17, 2, 14, 11, 8], 비디오테이프: [0], 탁상시계: [6, 0],
  // 귤은 주황(9)
  // 액자는 사진대로 청회색 도장 테·크림 대지 — 흰 팔레트에 계수로 칠한다.
  // 재떨이는 누름 유리 — 투명(흰색) · 청록(민트). 사진의 분홍 유리는 유리 계수를 곱하면 탁한 자주가 돼서 뺐다
  액자: [0], 귤: [9], 재떨이: [0, 13],
  // 콘센트는 팔레트 3(0xb0aca4) — 이 표에 「밝은 회색 콘센트·플라스틱」이라고
  // 적혀 있는 그 칸이다. 예전 압출 색(0xf2ece0)은 벽(0xfbf0d2) 대비 1.05:1 이라
  // **원리상 안 보였다.** 이 색은 1.6:1 이다. 사진(2026-09-14)의 판은 아이보리지만 그대로 따르면
  // 크림색 벽에 다시 묻힌다 — 회색을 지키고 형상에서 따뜻한 계수만 살짝 얹는다
  콘센트: [3],
  // ── 나머지 여섯 방의 가구 (손배치 전용) ─────────────────────
  // 나무 가구는 7(나무), 도기는 0(종이 흰색), 스테인리스는 6(은색).
  // 이불은 두 색을 준다 — 요와 걷어놓은 이불이 같은 색이면 층이 안 보인다
  // 우산꽂이는 사진대로 푸른 슬레이트빛 도자기 — 밝은 회색(3)에 계수를 곱한다. 흰 우산도 이 팔레트라야 희다
  신발장: [7], 우산꽂이: [3],
  // 아이 방 세 가구는 사진 기준 재작업에서 부품 계수가 색을 정한다(철판 · 청록 요 · 흰 엮음) — 흰 팔레트
  책상: [0], 이불: [0], '장난감 상자': [0],
  싱크대: [0], 냉장고: [0], 찬장: [0], 식탁: [0],
  // 욕조·찬장은 **팔레트를 흰색(0)으로 둔다.** 팔레트가 물건 색이면 유리문과
  // 파인 안쪽까지 그 색에 눌려서, 찬장이 통짜 나무 궤짝으로 욕조가 민트색 상자로
  // 보였다. 흰색에 계수를 곱하는 쪽이 부품끼리 밝기를 벌릴 수 있다
  욕조: [0], 변기: [0], 세면대: [0],
  // 뒷마당. 나무는 초록(11), 창고는 은색(6), 개집·평상은 나무(7)
  ...
  석등: [3], 물확: [3], 징검돌: [3], 대나무: [0], 소나무: [0], 게타: [0], 갈퀴: [0],
  // 정원돌 — 화강암 결은 계수(`GRANITE`)가 만든다. 팔레트는 중성 회색
  세로돌: [3], 가로돌: [3], 비스듬돌: [3],
  // ── 동네 맵 전용 ────────────────────────────────────────────
  // 병뚜껑에 검정(5)을 주면 주름과 인쇄를 통째로 삼킨다 — 은색으로 옮겼다
  꽃잎: [0, 15], 자갈: [3, 4, 2, 0], 병뚜껑: [6, 8, 11], 도토리: [0], 솔방울: [7],
  // 꽃은 팔레트를 흰색 하나로 둔다 — 빨강·노랑·민트를 곱하면 «초록 줄기»가 나올 수 없다.
  // 꽃잎 색은 계수가 낸다(`shapes.town.ts` 꽃 머리 주석).
  동전: [6], 꽃: [0], '연어 캔': [0], 쥐: [0], 골프공: [0],
  참새: [0], 페트병: [0], 모종삽: [0], 비둘기: [3], 삽: [0],
  // 연어 캔 · 페트병 · 모래성은 라벨 · 홍차 · 젖은 모래 색을 인쇄가 낸다 — 팔레트를 곱하면 다 뭉갠다.
  // 모래성은 주황(9)을 주면 사진의 «푸른 기 도는 회색 모래»가 사막이 된다.
  개밥그릇: [0], 양동이: [0], 모래성: [0], 삼각콘: [9], 개: [0],
  // ── World 맵 전용 ───────────────────────────────────────────
  // 타이어에 팔레트 검정(5)을 주면 DARK 정점색(0.18)과 곱해져 형체가 사라진다.
  // 오토바이가 검정이면 DARK 바퀴와 뭉쳐 덩어리가 되는 것과 같은 이유다.
  // 사진에서 잰 색은 계수와 인쇄가 낸다(`ref/벽돌·타이어·소화전·볼라드·입간판·자전거·오토바이/`) —
  // 벽돌은 적갈(17)이 아니라 누런 모래색이고, 소화전은 빨강(8)이 아니라 하코다테식 진한 노랑이며,
  // 볼라드는 노랑(10)이 아니라 주철 검정, 입간판은 나무(7)가 아니라 바랜 크림색이다.
  벽돌: [0], 축구공: [0], 타이어: [0],
  소화전: [10], 볼라드: [0], 입간판: [0], 정글짐: [0],
  // 우체통 빨강 · 표지판 흰 바탕 · 벤치 회베이지 널 · 그네 노란 이음쇠 · 자판기 흰 몸통 ·
  // 미끄럼틀 도금 강판 · 정글짐 노란 한 층은 모두 계수와 인쇄가 낸다 — 팔레트를 곱하면 다 뭉갠다.
  자전거: [0], 오토바이: [0], 우체통: [0], 표지판: [0], 드럼통: [11, 8],
  벤치: [0], 그네: [0], 자판기: [0], 미끄럼틀: [0], 사람: [0],
  // 가로수에 초록(11)을 주면 «줄기까지» 초록이 된다(1회차 트랙 D 「줄기가 잎과 같은 초록」).
  // 정점색은 팔레트에 곱해지므로 초록 팔레트 아래에서 갈색 줄기를 만들 방법이 없다.
  승용차: [0, 8, 14], 가로수: [0],
};
 ...
 * 게타·갈퀴만 뒷마당 표에 오른다.
 */
export const SHAPE_IDS_GARDEN = [
  // 정원돌 — **레퍼런스가 「가장 중요한 요소」라고 하는 것**
  '세로돌', '가로돌', '비스듬돌',
...
/** 동네 구역도 경계가 같고 이름만 다르다 */
const townTable = houseTable;

/**
 * **구역마다 자기 물건.** `TOWN_ROOMS`(stage.town.ts)가 구역 id 로 골라 문다.
 *
 * ## 집 맵이 이미 겪고 고친 병이다
 *
 * 표가 스테이지당 하나면 **크기만으로 이름이 정해진다.** 20cm 짜리는 공사장에
 * 있든 캠프장에 있든 같은 일곱 이름 중에서 뽑혔고, 그래서 **공사장에 도토리와
 * 달팽이가, 캠프장에 병뚜껑이, 야구장에 찻잔이** 깔렸다.
 * 집 맵은 「부엌에 크레용이 깔렸다」로 이 병을 겪고 `ROOM_TABLES` 로 고쳤다.
 *
 * ## 경계는 손대지 않는다
 *
 * 1·2·4·8·15·30·60·120cm — 일곱 칸 그대로다. **경계는 사다리**라서 구역마다
 * 바꾸면 `ladder` 가 구역마다 다른 자를 대게 된다. 바뀌는 건 각 칸의 이름뿐이다.
 *
 * ## 구역이 안 쓰는 칸도 채운다
 *
 * 시작 마당은 `sizeMax` 0.16 이라 30cm 위 두 칸이 안 뽑힌다. 그래도 안 비운다 —
 * 범위를 넓히는 날 빈 배열을 만나면 `labels[(rand()*0)|0]` 가 `undefined` 가 된다.
 *
 * ## 바깥 세 구역은 큰 것만 쓴다
 *
 * 야구장·강·언덕은 `sizeMin` 이 0.3~0.5 라 **마지막 두 칸만** 뽑힌다.
 * 거기에 찻잔·사과를 두면 12m 공 앞에 0.3m 로 부풀린 찻잔이 깔린다 —
 * 거리 물건(자전거·드럼통·벤치·타이어)이 그 자리에 맞다.
 */
export const TOWN_ROOM_TABLES: Record<string, LabelTable> = {
  /** 시작 마당 — 집 마당이다. 살림 흔적과 마당 것이 섞인다 */
  yard: townTable([
    ['개미', '꽃잎', '자갈', '클립'],
    ['동전', '압정', '사탕', '나사'],
    ['꽃', '청개구리', '골프공', '건전지'],
  ...
  /** 비둘기 광장 — 사람이 앉았다 가는 데. 흘린 것과 버린 것 */
  plaza: townTable([
    ['병뚜껑', '클립', '꽃잎', '단추'],
    ['동전', '사탕', '성냥', '지우개'],
    ['꽃', '골프공', '연어 캔', '성냥갑'],
  ...
  /** 상점가 — 가게 앞. 포장과 잔돈이다. 도토리·솔방울이 여기 있을 이유가 없다 */
  shops: townTable([
    ['병뚜껑', '클립', '단추', '압정'],
    ['동전', '사탕', '성냥', '지우개'],
    ['연어 캔', '건전지', '성냥갑', '골프공'],
  ...
  /** 공사장 — 연장과 부속. 꽃잎·달팽이가 여기 있을 이유가 없다 */
  site: townTable([
    ['자갈', '클립', '병뚜껑'],
    ['나사', '압정', '동전'],
    ['건전지', '골프공', '연어 캔'],
  ...
  // ── 바깥 세 구역 (별 8 전용) ──────────────────────────────
  // `sizeMin` 이 0.3~0.5 라 **마지막 두 칸만** 실제로 뽑힌다.
  // 앞의 다섯 칸은 「안 쓰지만 비우지 않는다」는 규약 때문에 채운다.
  /** 야구장 — 공과 길가 설비 */
  field: townTable([
    ['자갈', '병뚜껑', '클립'],
    ['동전', '나사', '성냥'],
...
export const ROOM_TABLES: Record<string, LabelTable> = {
   ...
   * 뒷마당 — 일본식 정원.
   *
   * **페트병·연어 캔·병뚜껑·나사·신문·모래성·골프공을 뺐다.** 실측하니 마당에서
   * 제일 많은 셋이 페트병 28 · 연어 캔 25 · 양동이 23 이었고, 그게 정원이 아니라
   * 재활용 수거장으로 읽히던 이유다.
   *
   * **버킷 수(일곱)와 크기 폭은 그대로다** — 사다리 칸은 이름이 아니라 크기가
   * 채운다. 자리만 정원 물건(자갈·꽃잎·게타·갈퀴)으로 갈아 끼웠다.
   *
   * 석등·물확·대나무·소나무·징검돌은 **여기 없다.** 넣으면 마당에 석등이
   * 스무 개 흩뿌려진다 — `나무`·`개집`·`창고` 가 이미 손배치 전용인 것과 같다.
   */
  yard: houseTable([
    ['개미', '자갈', '도토리', '꽃잎'],
    ['솔방울', '자갈', '동전', '단추'],
    ...
    ['양동이', '개밥그릇', '화분', '갈퀴', '삽'],
    /**
     * **의자·스툴을 뺐다.** 실측하니 이 버킷이 `의자` 를 1.04m, `스툴` 을 0.90m 로
     * 뽑아 **갈퀴로 그은 자갈 마당 위에** 앉히고 있었다 — 화면에서 정원 한가운데
     * 거대한 갈색 원반 넷이 그것이다. 실내 가구가 마당에 나와 있을 이유가 없다.
     *
     * 정원에서 제일 큰 물건은 **손배치가 맡는다**(석등 1.15 · 소나무 1.45 ·
     * 대나무 1.90 · 창고 1.80 · 나무 2.60). 흩뿌림은 그 밑을 채우면 된다.
     */
    ['개', '고양이', '물뿌리개', '화분', '개밥그릇'],
  ]),
};
...
export interface ObjectSpec {
  ...
  /** 왕복 구간 `[x0, z0, x1, z1, 속도]`. `StageProp.patrol` 이 그대로 실려 온다 */
  patrol?: readonly [number, number, number, number, number];
}
