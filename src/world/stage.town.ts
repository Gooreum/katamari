...
// 컨테이너 주황(C_CONTAINER)은 없앴다 — 컨테이너가 형상이 되면서
// 색을 형상의 정점색이 지고, 여기서 줄 일이 없어졌다.
...
const TOWN_PROPS: readonly StageProp[] = [
  ...
  // ── 호숫가 · 길가: 상자에서 형상으로 ───────────────────────
  { label: '오리배', x: 10.2, z: 9.1, size: 1.8, rotY: -0.7 },
  { label: '보트', x: 12.2, z: 9.1, size: 1.7, rotY: 0.55 },
  { label: '자전거 보관대', x: 7.6, z: -18.0, size: 1.9, rotY: Math.PI / 2 },
  { label: '모래 포대', x: -13.9, z: -5.3, size: 1.5, rotY: 0.3 },

  // ── 거리 설비: 상자에서 형상으로 ───────────────────────────
  // 자리와 크기는 옮기기 전 `block()` 사각형의 **중심과 높이 그대로**다
  // 정자 발자국이 거의 2m 를 꽉 채워서 x 1.1 에 두니 겹쳤다 — 동쪽으로 물렸다
  { label: '분수대', x: 1.9, z: 11.1, size: 1.6 },
  { label: '정자', x: -0.6, z: 10.2, size: 2.0, rotY: 0.1 },
  { label: '가로등', x: -0.6, z: 12.6, size: 2.5 },
  { label: '가로등', x: 14.0, z: 9.1, size: 2.5 },
  { label: '게시판', x: 3.4, z: 9.4, size: 1.9, rotY: -0.5 },
  { label: '게시판', x: -3.4, z: -15.6, size: 2.1, rotY: Math.PI },
  { label: '전봇대', x: -8.2, z: -12.6, size: 2.4 },
  { label: '전봇대', x: 7.8, z: -20.0, size: 2.4 },

  // ── 캠프장 · 공사장: 상자에서 형상으로 ─────────────────────
  // 자리와 크기는 옮기기 전 `block()` 사각형의 **중심과 높이 그대로**다 —
  // 이 값들이 별 5 의 1.2~2.7m 사다리를 지고 있다
  { label: '텐트', x: -12.6, z: 4.5, size: 2.2, rotY: 0.35 },
  { label: '텐트', x: -9.1, z: 8.9, size: 2.0, rotY: -0.6 },
  { label: '장작더미', x: -12.0, z: 7.4, size: 1.7, rotY: 0.2 },
  { label: '캠프파이어', x: -10.4, z: 5.8, size: 1.8 },
  { label: '컨테이너', x: -12.15, z: -7.1, size: 2.6, rotY: 0.12 },
  { label: '컨테이너', x: -8.15, z: -4.7, size: 2.6, rotY: -0.25 },
  { label: '파이프 더미', x: -9.6, z: -7.6, size: 2.0, rotY: 0.5 },
  ...
  // 차가 3.6m 라 구간 끝에서 발자국이 문까지 닿는다. 광장 동문(x=6)과
  // 야구장 문(x=16) 앞 1m 를 비우도록 구간을 양쪽에서 줄였다
  { label: '승용차', x: 8.8, z: 13.2, size: 3.6, patrol: [8.8, 13.2, 13.2, 13.2, 1.8] },
