...
/**
 * **아주 어두운 팔레트 위의 금속.** `WRAP`(1.55)보다 한 단계 더 밝다.
 *
 * 대비는 계수 차이가 아니라 «계수 차이 × 팔레트 밝기»다. 검정 팔레트
 * (0x2f2e2c, 밝기 0.18)에서는 흰색과 `WRAP` 의 차이가 0.55 × 0.18 = **0.099** 밖에
 * 안 된다 — 문턱(0.15) 아래다. 검은 텔레비전의 크롬 안테나가 그래서 안 보였다.
 * 밝은 팔레트에서는 흰색으로 포화되는데, 크롬은 원래 그렇게 보인다.
 */
export const SHINE: RGB = [2.6, 2.55, 2.4];
