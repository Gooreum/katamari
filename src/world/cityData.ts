...
export interface CityRug {
  ...
  /**
   * 이 깔개는 **타일이 아니라 이 자리를 위해 그린 한 장**이다.
   * `repeat` 을 1×1 로 두어 그림이 통째로 이 사각형을 덮는다.
   *
   * 카레산스이의 갈퀴 자국(砂紋)은 **돌 둘레의 동심원**이라 위치에 매여 있다 —
   * 1.8m 타일을 반복해서는 못 그린다. 그래서 이 한 장짜리가 필요했다.
   * 알파 0 인 픽셀은 `alphaTest` 가 버리므로 **직사각형을 벗어난 모양**도 된다.
   */
  readonly fit?: boolean;
  /**
   * 이 깔개 위에는 **소품을 안 놓는다**(`World.buildBlocked` 가 막는다).
   *
   * 레퍼런스의 「카레산스이는 뺄셈이다 — 빈 공간(空)이 요소다」를 기계로 만든 것이다.
   * 갈퀴로 그은 자갈 위에 양동이가 널려 있으면 그건 갈퀴질이 아니다.
   */
