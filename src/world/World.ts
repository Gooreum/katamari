...
export class World {
  ...
  /**
   * 지면 — 도로가 **아닌** 곳. 블록 안쪽, 주차장, 공터.
   *
   * 예전에는 초록 잔디 체크무늬(#7fb069)였다. Roads.ts 가 실제 도로 4,554개를
   * 그리기 시작한 뒤에도 그 사이가 전부 잔디라 서울이 잔디밭 위에 있었다.
   *
   * **1m 격자는 유지한다** — 5cm 공에게는 이게 유일한 절대 크기 기준이다.
   * 다만 체크무늬가 아니라 콘크리트 줄눈으로 바꾼다. 실제 포장이 판 단위로 줄눈을
   * 갖고 있어서, 같은 크기 정보를 주면서 잔디로는 안 보인다.
   *
   * 색은 도로(0x5f6268 ~ 0xa8a091)보다 **어둡게** 잡는다.
  ...
  private buildGround(): Mesh {
    ...
    for (let i = 0; i < 240; i++) {
      const v = 74 + ((i * 7919) % 14);
      cx.fillStyle = `rgb(${v},${v - 1},${v - 3})`;
