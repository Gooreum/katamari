...
export const SMALL_BUILDERS: Record<ShapeIdSmall, () => BufferGeometry> = {
  ...
  캐러멜: () => assemble([
     ...
     *
     * **색이 너무 짙었다.** 적갈(0x8a4f2a)을 썼는데 사진의 캐러멜은 그보다 훨씬
     * 옅은 «우유 섞인 밝은 갈색»이다. 짙은 갈색은 초콜릿이지 캐러멜이 아니다.
     * 팔레트를 크라프트(0xc7a878)로 옮겼다.
     *
     ...
     */
    part(soft(0.64, 0.40, 0.50, 0.17), WHITE, [0, 0.20, 0], undefined, TILE.TOFFEE),
    /**
     * **위아래 「자른 면」 판을 붙였다가 뺐다.** 얇은 판이 몸통 위아래에 얹히면
     * 그 선이 **상자 뚜껑 이음선**으로 읽힌다 — 실제로 골판지 상자가 됐다.
     * 모따기 0.09 도 너무 각져서 같은 방향으로 밀었다. 0.17 이 그 사이다.
     *
     * 이 형상에서 세 번 확인한 것: **캐러멜과 상자를 가르는 건 모서리가 아니라
     * «선이 없는 것»이다.** 이음선·테두리·판 자국이 하나라도 있으면 상자로 간다.
     */
  ]),
  /**
