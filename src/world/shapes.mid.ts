...
export const MID_BUILDERS: Record<ShapeIdMid, () => BufferGeometry> = {
  ...
  달팽이: () => assemble([
     ...
     */
    /**
     * **껍데기는 돔이다. 바닥에 깔린 도넛이 아니다.**
     * 근거: `.design-bounce/ref/달팽이/` (위키미디어)
     *
     * 앞의 것은 나선을 «옆으로» 눕혀서 지름만 크고 높이가 없었다. 정답을 모르는
     * 판정자에게 보였더니 근거를 「매끈한 갈색 도넛 고리」라고 적었다 — 나선이
     * 안 보인다는 뜻이다. (1차에는 「둥근 나선 껍데기」라고 답했는데, 그건
     * 달팽이의 «정의»를 알고 채워 읽은 것이었다.)
     *
     * 사진에서 잰 것: 껍데기 **높이가 지름과 거의 같다.** 그래서 고리를 옆으로
     * 늘어놓지 않고 **위로 쌓는다** — 반지름이 줄면서 y 가 오른다. 그러면 옆에서
     * 봐도 나선이 층으로 보이고, 위에서 봐도 소용돌이가 보인다.
     *
     * **껍데기를 한 번 더 줄였다.** 돔으로 바꾼 뒤에도 껍데기가 몸통보다 커서
     * 「껍데기에 머리가 붙은 것」으로 보였다. 사진에서 잰 비율은
     * **껍데기 지름 : 몸통 전체 길이 = 1 : 1.8** 이다 — 몸통이 훨씬 길다.
     */
    part(new TorusGeometry(0.205, 0.088, 5, 16), [0.52, 0.40, 0.26], [0.19, 0.115, 0], LIE_Z),
    part(new TorusGeometry(0.155, 0.077, 5, 13), [0.62, 0.48, 0.31], [0.205, 0.215, 0], LIE_Z),
    part(new TorusGeometry(0.105, 0.064, 4, 11), [0.74, 0.58, 0.38], [0.22, 0.298, 0], LIE_Z),
    part(new TorusGeometry(0.057, 0.049, 4, 8), [0.88, 0.70, 0.46], [0.235, 0.361, 0], LIE_Z),
    // 나선이 도는 꼭지 — 맨 위 한 점. 이게 있어야 「감겨 올라간 끝」이 된다
    part(new SphereGeometry(0.040, SEG.TINY, 4), [1.02, 0.84, 0.58], [0.243, 0.401, 0]),
    /**
     * 껍데기 입구에 둘린 **옅은 크림색 테두리.** 사진에서 뚜렷하게 보이는 것이고,
     * 이게 있어야 껍데기가 «속이 빈 것»으로 읽힌다.
     */
    part(new TorusGeometry(0.197, 0.024, 4, 14), [1.24, 1.20, 1.08], [0.19, 0.063, 0], LIE_Z),
    // 발 — 껍데기보다 «앞으로» 길게. 최장축이 발이라야 「기어간다」가 된다
    part(soft(0.80, 0.105, 0.20, 0.45), [1.10, 1.06, 0.96], [-0.26, 0.052, 0]),
    part(new SphereGeometry(0.125, SEG.SMALL, 6).scale(1.05, 0.90, 0.95), [1.10, 1.06, 0.96],
      [-0.58, 0.098, 0]),
    /**
