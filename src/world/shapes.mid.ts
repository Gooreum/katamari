...
export const MID_BUILDERS: Record<ShapeIdMid, () => BufferGeometry> = {
  ...
  달팽이: () => assemble([
     ...
     */
    part(new TorusGeometry(0.245, 0.105, 5, 16), [0.52, 0.40, 0.26], [0.10, 0.135, 0], LIE_Z),
    part(new TorusGeometry(0.185, 0.092, 5, 13), [0.62, 0.48, 0.31], [0.12, 0.255, 0], LIE_Z),
    part(new TorusGeometry(0.125, 0.076, 4, 11), [0.74, 0.58, 0.38], [0.14, 0.355, 0], LIE_Z),
    part(new TorusGeometry(0.068, 0.058, 4, 8), [0.88, 0.70, 0.46], [0.16, 0.430, 0], LIE_Z),
    // 나선이 도는 꼭지 — 맨 위 한 점. 이게 있어야 「감겨 올라간 끝」이 된다
    part(new SphereGeometry(0.048, SEG.TINY, 4), [1.02, 0.84, 0.58], [0.17, 0.478, 0]),
    /**
     * 껍데기 입구에 둘린 **옅은 크림색 테두리.** 사진에서 뚜렷하게 보이는 것이고,
     * 이게 있어야 껍데기가 «속이 빈 것»으로 읽힌다.
     */
    part(new TorusGeometry(0.235, 0.028, 4, 14), [1.24, 1.20, 1.08], [0.10, 0.075, 0], LIE_Z),
    // 발 — 껍데기보다 «앞으로» 길게. 최장축이 발이라야 「기어간다」가 된다
    part(soft(0.70, 0.12, 0.23, 0.45), [1.10, 1.06, 0.96], [-0.24, 0.060, 0]),
    part(new SphereGeometry(0.145, SEG.SMALL, 6).scale(1.0, 0.88, 0.95), [1.10, 1.06, 0.96],
      [-0.50, 0.115, 0]),
    /**
     * **더듬이는 넷이다.** 사진에서 확인했다 — 눈이 달린 «긴 위 더듬이» 둘과
     * 그 아래 짧은 «아래 더듬이» 둘. 지금까지 둘만 달아서 개미와 구별이 안 됐다.
     */
    ...([0.065, -0.065] as const).flatMap((z) => [
      part(new CylinderGeometry(0.022, 0.016, 0.27, 6), [1.10, 1.06, 0.96],
        [-0.58, 0.255, z], [0, 0, 0.42]),
      part(new SphereGeometry(0.046, SEG.TINY, 4), INK, [-0.645, 0.375, z]),
      // 아래 더듬이 — 짧고 앞으로 뻗는다. 위 더듬이와 길이가 확실히 달라야 넷으로 보인다
      part(new CylinderGeometry(0.016, 0.011, 0.13, 5), [1.10, 1.06, 0.96],
        [-0.615, 0.085, z * 1.35], [0, 0, 1.15]),
    ]),
  ]),
