 ...
 *
 * **심시티 전환으로 700에서 500으로 내렸다.** 건물 삼각형이 파라펫·옥상 설비 때문에
 * 27만 → 44만으로 늘면서 최악 청크 재빌드 중앙값이 8.54ms가 됐다 (프레임 예산 16.7ms의 절반,
 * 자체 상한 8ms 초과). 표본 11개로 쟀다 — 5개로는 6.99ms가 나와서 통과처럼 보였다.
 *
 * 다른 후보와 재보고 골랐다:
 *   원통 8각→6각    6.3% 절감 · 물탱크가 각져 보인다
 *   파라펫 문턱 6→10m 2.5% 절감 · 1,124채의 옥상 테두리가 사라진다
 *   CHUNK 700→500   46% 절감 · **보이는 것은 하나도 안 바뀐다**
 * 드로우콜은 61 → 107로 늘지만 상한 120 안이다. 붙은 물체가 최대 110을 따로 쓰는데,
 * 그건 흡수가 진행된 뒤의 이야기고 그때는 도시 건물이 줄어 있다.
 */
const CHUNK = 500;
