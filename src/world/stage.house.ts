...
export const HOUSE_PROPS: readonly StageProp[] = [
  // ── 복도 ────────────────────────────────────────────────
  { label: '신발장', x: -0.67, z: -2.90, size: 0.80, rotY: Math.PI / 2 },
  { label: '우산꽂이', x: 0.65, z: -3.10, size: 0.48 },

  // ── 아이 방 ─────────────────────────────────────────────
  // 책상·식탁은 밑이 뚫린다 — 밥상과 같다(상판만 충돌을 잡는다)
  { label: '책상', x: -3.70, z: -5.325, size: 1.00, underPass: 0.42 },
  { label: '의자', x: -3.70, z: -4.70, size: 0.45 },
  { label: '이불', x: -1.95, z: -4.65, size: 1.90, rotY: Math.PI / 2 },
  { label: '장난감 상자', x: -4.05, z: -2.85, size: 0.50 },
  // **rotY 부호에 주의.** 형상은 +Z 로 열린다 — −π/2 를 주면 선반이 서벽(x −4.5)
  // 쪽으로 열려서 «벽을 보고 선 책장»이 된다. 화면으로만 잡히는 결함이다
  { label: '책장', x: -4.20, z: -4.05, size: 1.05, rotY: Math.PI / 2 },

  // ── 부엌 ────────────────────────────────────────────────
  { label: '싱크대', x: -3.475, z: -8.10, size: 1.75 },
  { label: '냉장고', x: -2.00, z: -8.10, size: 1.55 },
  { label: '찬장', x: -4.20, z: -7.05, size: 1.45, rotY: Math.PI / 2 },
  { label: '식탁', x: -3.35, z: -6.55, size: 1.20, underPass: 0.55 },

  // ── 화장실 ──────────────────────────────────────────────
  // 1.8m × 1.8m 짜리 방이라 셋만 넣어도 꽉 찬다
  { label: '욕조', x: 2.09, z: -8.085, size: 0.98 },
  { label: '변기', x: 2.34, z: -7.125, size: 0.72, rotY: -Math.PI / 2 },
  // 문 앞을 비켜야 한다. 처음엔 (1.25, −7.10) 이었는데 형상이 압출 블록보다
  // 8cm 넓어서 문 앞 통로(z ≤ −7.30)를 4cm 물었다
  { label: '세면대', x: 1.30, z: -7.06, size: 0.74, rotY: Math.PI / 2 },
   ...
   * 목표는 별4의 1m 가 최대다 — **아무도 안 밟는 칸**이다. 도구를 만족시키자고
   * 마당에 2m 짜리 물건 아홉 개를 더 세우는 건 사다리를 메우는 게 아니라 마당을
   * 망치는 것이다. 이번 작업은 형상만 바꾼다.
   */
