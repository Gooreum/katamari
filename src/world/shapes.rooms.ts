...
export const ROOM_BUILDERS: Record<ShapeIdRooms, () => BufferGeometry> = {
  ...
  욕조: () => {
    ...
    return assemble([
      ...
      // 안쪽 벽을 **그늘색**으로 내린다. 밝은 도기색이면 카메라 기울기가 14° 뿐이라
      // 안쪽 먼 벽이 «윗면 판»처럼 보여 통이 통째로 닫혀 보인다 — 네 회차 내내 「상자」였다
      ...basin(TW, TD, TH - 0.02, WALL, [0.34, 0.46, 0.48], AQUA)
      ...
      // 물빛이 아니라 «들여다보이는 깊이»다
      part(new BoxGeometry(TW - WALL * 2 - 0.012, 0.012, TD - WALL * 2 - 0.012), [0.45, 0.75, 1.10], [TX, TH * 0.50, 0]),
      // ③ 뚜껑 판 — 사진처럼 왼쪽만 덮고, **한 장은 비스듬히 세워 기대 놓는다.**
      // 평평하게 덮으면 위에서 내려다보지 않는 한 「뚜껑 덮인 상자」와 구별되지 않는다.
      // 비껴 세운 판 한 장이 「이 통은 열려 있다」를 옆에서도 말해 준다(사진도 그렇게 세워져 있다)
      part(new BoxGeometry(TW * 0.30, 0.03, TD + 0.02), [0.70, 0.74, 0.72], [-0.5 + TW * 0.15, TH + 0.017, 0]),
      part(new BoxGeometry(TW * 0.40, 0.03, TD - 0.06), [0.82, 0.86, 0.84],
        [-0.5 + TW * 0.16, TH + 0.10, 0], [0, 0, -0.62]),
