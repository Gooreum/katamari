...
export const ROOM_BUILDERS: Record<ShapeIdRooms, () => BufferGeometry> = {
  ...
  욕조: () => {
    ...
    return assemble([
      ...
      // ③ 뚜껑 판 — 사진처럼 왼쪽만 덮는다.
      // 한 장을 비스듬히 세워 봤다가 **되돌렸다.** 들린 판이 「뚜껑 들린 사무기기(복사기)」로 읽혀
      // 더 나빠졌다(2026-09-17) — 청록 상자 위에 들린 뚜껑은 욕조보다 복사기에 가깝다
      part(new BoxGeometry(TW * 0.30, 0.03, TD + 0.02), [0.70, 0.74, 0.72], [-0.5 + TW * 0.15, TH + 0.017, 0]),
      part(new BoxGeometry(TW * 0.26, 0.03, TD - 0.06), [0.82, 0.86, 0.84], [-0.5 + TW * 0.14, TH + 0.047, 0]),
