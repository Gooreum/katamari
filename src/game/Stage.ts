...
/**
 * 원작 값. 여덟 판이 **세 맵을 재사용**한다 — 1·2·4는 타케다 저택, 3·5·8은 동네,
 * 6·7은 World의 어친타운.
 *
 * 이름이 `HOUSE_STAGES`였는데 절반이 동네라 이미 거짓말이었다. `STAGES`로 고쳤다.
 *
 * ## 시작 크기 — 6·7번 50cm, 8번 10cm만 확인됐다
 *
 * **6·7번이 "Starting Size 50cm", 8번이 10cm**로 명시된 걸 찾았다. 스테이지별 시작
 * 크기가 실제로 있다는 뜻이다. 그런데 **2~5번의 값은 어느 자료에도 없다.**
 * 그래서 그 넷은 1번 값(5cm)을 그대로 둔다 — 지어내지 않는다.
 *
 * ## 6번은 확증도가 낮다 — 두 군데가 그렇다
 *
 * **무대**: 명시한 자료를 못 찾았다. 선물 위치가 "주유소 뒤 교정, 정글짐 옆"인데
 * 그건 어친타운의 특징(양 끝의 공원과 학교)이라 World로 잡았다. **추정이다.**
 * **제한시간**: 10분과 11분으로 엇갈렸다. 더 구체적인 쪽(11분 = 660초)을 택했다.
 *
 * ## 8번 무대는 Town이다 — World가 아니다
 *
 * 8번의 Roadway는 **Town 구역**이다 (Urchin Town이 World 구역인 것과 반대다).
 * 그래서 8번은 새 맵이 아니라 동네 맵을 재사용한다 — 우리 동네에 이미
 * 「호숫가 도로」가 있다.
 *
