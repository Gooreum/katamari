...
export class Game {
  ...
  constructor(
     ...
     *
     * ## 손잡이가 «둘»인데 하나만 돌리고 있었다
     *
     * 위 판단은 **세기**만 놓고 한 것이고 `groundColor` 는 안 건드렸다.
     * `0xd7e8bc` 는 밝기 0.87 로 **거의 흰색**이다 — 아래를 보는 면(그림자 쪽)까지
     * 그 색으로 밝게 메우니 세기를 아무리 만져도 명암이 안 벌어진다.
     * 실측: 게임 프레임의 밝기 10~90% 폭이 **255 중 66** 이었다.
     *
     * 땅색을 짙게(`0x3a3524`, 밝기 0.19) 하고 세기를 0.42 로 내린 뒤 해를 2.35 로
     * 올려 총광량을 지켰다. 셋을 같이 돌린 실측:
     *
     * ```
     *   하늘광 0.75 · 땅색 0xd7e8bc · 해 1.65   폭  77   제일 어두운 1% 평균 25
     *   하늘광 0.55 · 땅색 0x8a8470 · 해 2.00   폭  90                      23
     *   하늘광 0.42 · 땅색 0x3a3524 · 해 2.35   폭 109                      18   ← 채택
     * ```
     *
     * **검정은 여전히 안 뭉갠다** — 화면에서 제일 어두운 1% 가 평균 18 이다(0 이 아니다).
     * 위 주석의 걱정은 옳았고, 그래서 이번에도 **재고 나서** 옮겼다.
     */
    this.scene.add(new HemisphereLight(0xffffff, 0x3a3524, 0.42));
