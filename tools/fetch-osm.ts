...
const PRESETS = {
  ...
  /**
   * 문정동 — 다른 넷과 달리 **동네 한 조각**이다. 앞의 넷이 1.2~2.8km 짜리 "지역"이라면
   * 여기는 블록 서넛이다. 「집 문 열고 나와서 내 동네를 먹는다」가 이 맵의 전제다.
   *
   * **반경 300m 는 작아서 고른 게 아니라 데이터가 제일 좋아서 골랐다.**
   * 수집 전에 반경 700m 를 미리 받아 후보를 재 봤다:
   *
   *   집 300m    건물  62채 · 도로  53개 · 높이 정보 51%
   *   중간 400m  건물 295채 · 도로 127개 · 높이 정보 18%
   *   문정역500m 건물 464채 · 도로 187개 · 높이 정보 20%
   *
   * 넓힐수록 건물은 느는데 `building:levels` 가 있는 비율이 반토막 난다 — 문정동
   * 빌라촌은 OSM 에 층수가 거의 없어서, 넓힐수록 `heightFromTags` 의 종류별
   * 추정값(주택 7m · 기타 12m)으로 뭉친 건물이 늘어 **사실성이 오히려 나빠진다.**
   * 사다리도 300m 가 제일 고르다 (8m:11 · 16m:18 · 32m:17 · 64m:13 · 128m:3채).
   *
   * 반경 300m → 지면 평면 780m (`World.ts:353`, radius × 2.6),
   * 소품 2,211개 · 배치 반경 300m (`World.ts:388-393` 이 reach 로 자동 스케일).
   */
  munjeong: {
    name: '문정동', slug: 'munjeong',
    origin: { lat: 37.48231, lon: 127.12211 },  // 문정아이파크 (법원로4길 6)
    // **원점이 아파트 «안»이라 spawn 을 따로 준다.** 건물 AABB + 3m 밖에 있는 가장
    // 가까운 도로점이 남서쪽 50m 의 보행로다. `findClearSpawn`(World.ts:936) 이
    // 런타임에 나선으로 보정하긴 하지만, 그건 "틀린 값을 구제하는" 경로다 —
    // 처음부터 맞는 값을 주면 그 탐색이 아예 안 돈다.
    radius: 300, spawn: { x: -24, z: 44 },
  },
} as const;
